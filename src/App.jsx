...
// Calcula saldo real da conta: inicial + receitas - despesas débito (cartão não conta)
const getSaldoReal = (conta, allTx) => {
  const entradas = allTx.filter(t => t.contaId === conta.id && t.tipoFluxo === "Receitas").reduce((s,t) => s + t.valor, 0);
  const saidas   = allTx.filter(t => t.contaId === conta.id && t.tipoFluxo !== "Receitas" && !t.cartaoId).reduce((s,t) => s + t.valor, 0);
...
function HomeTab({txMes,receitas,despesas,saldo,orcamento,evolucao,contas,cartoes,objetivos,transactions}){
  const totalContas=contas.reduce((s,c)=>s+getSaldoReal(c,transactions),0);
...
function PatrimonioTab({patrimonio,onSavePatrimonio,contas,allTx}){
  ...
  const inp={background:C.card2,border:`1px solid ${C.border}`,color:C.text,padding:"10px 12px",borderRadius:8,fontSize:13,width:"100%",boxSizing:"border-box",outline:"none"};
  const totalContas=contas.reduce((s,c)=>s+getSaldoReal(c,allTx||[]),0);
