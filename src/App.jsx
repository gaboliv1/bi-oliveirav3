...
function FormModal({onClose,onSave,contas,cartoes}){
  const[form,setForm]=useState({data:hoje(),valorNum:0,valorRaw:"",fluxo:"Saída",tipoFluxo:"Necessidades básicas",classificacao:"Supermercado",observacao:"",contaId:"",cartaoId:"",parcelado:false,parcelas:"2"});
  ...
  const salvar=async()=>{
    ...
    await onSave({
      ...
      observacao:form.observacao, mesAno:getMA(form.data),
      contaId:form.contaId||null,
      cartaoId:form.fluxo==="Saída"?(form.cartaoId||null):null,
      parcelado:form.parcelado&&!!form.cartaoId,
      parcelas:form.parcelado&&form.cartaoId?np:1,
      valorParcela:vp,
      parcelasPagas:[true,...Array(np-1).fill(false)],
    });
    ...
    <div style={{position:"fixed",inset:0,background:"#000c",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div style={{background:C.card,width:"100%",maxWidth:480,borderRadius:"20px 20px 0 0",padding:"16px 18px 36px",maxHeight:"94vh",overflowY:"auto"}}>
        ...
        <div style={{marginBottom:4}}>
          ...
          <ValorInput
            value={form.valorRaw}
            cor={form.fluxo==="Saída"?C.red:C.green}
            onChange={(num,raw)=>{setErro("");setForm(p=>({...p,valorNum:num,valorRaw:raw}));}}
          />
          <div style={{color:C.muted,fontSize:9,marginTop:3}}>Use vírgula para centavos: 1.500,75 ou 150,00</div>
        ...
        <button onClick={salvar} disabled={busy} style={{width:"100%",background:busy?"#333":`linear-gradient(135deg,${C.blue},${C.blue2})`,border:"none",borderRadius:12,padding:"16px",color:"#fff",fontWeight:800,fontSize:15,cursor:busy?"not-allowed":"pointer",boxShadow:`0 4px 20px ${C.blue}44`}}>
  ...
  return(
    <div>
      <div style={{background:`linear-gradient(135deg,${C.card2},${C.card3})`,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px",marginBottom:10,textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{color:C.muted,fontSize:9,textTransform:"uppercase",letterSpacing:2,marginBottom:4}}>Saldo Total em Contas</div>
        <div style={{color:C.gold,fontSize:32,fontWeight:900,fontFamily:"Georgia,serif"}}>{fmtR(totalContas)}</div>
        <div style={{color:C.muted,fontSize:10,marginTop:4}}>💳 Faturas abertas: <span style={{color:faturaTotal>0?C.red:C.green}}>{fmtR(faturaTotal)}</span></div>
      </div>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",marginBottom:10}}>
          ...
          {contas.map(c=>(
            <div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingBottom:8,marginBottom:8,borderBottom:`1px solid ${C.border}22`}}>
              ...
              <div style={{color:C.gold,fontSize:13,fontWeight:800}}>{fmtR(c.saldo||0)}</div>
            </div>
          ))}
        </div>
      )}
...
function ContasTab({contas,onSaveContas,allTx}){
  ...
  return(
    <div>
      ...
      {contas.map(c=>{
        const delta=allTx.filter(t=>t.contaId===c.id).reduce((s,t)=>t.tipoFluxo==="Receitas"?s+t.valor:s-t.valor,0);
        return(
          <div key={c.id} style={{background:C.card,border:`1px solid ${C.border}`,borderLeft:`4px solid ${c.cor||C.blue}`,borderRadius:10,padding:"12px 14px",marginBottom:8}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                ...
                <div style={{fontSize:10,color:C.muted}}>{c.tipo}{c.saldoInicial>0?` · Inicial: ${fmtR(c.saldoInicial)}`:""}</div>
                {delta!==0&&<div style={{fontSize:10,color:delta>0?C.green:C.red,marginTop:2}}>{delta>0?"▲":"▼"} {fmtR(Math.abs(delta))} lançado</div>}
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:18,fontWeight:800,color:C.gold}}>{fmtR(c.saldo||0)}</div>
...
// ── PATRIMÔNIO TAB ────────────────────────────────────────────────────────────
function PatrimonioTab({patrimonio,onSavePatrimonio,contas}){
  const[showAdd,setShowAdd]=useState(false);
    ...
    <div>
      ...
      {totalContas>0&&(
        <div style={{background:`linear-gradient(135deg,${C.card2},${C.card3})`,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px",marginBottom:10,textAlign:"center"}}>
          <div style={{color:C.muted,fontSize:9,textTransform:"uppercase",letterSpacing:1.5,marginBottom:4}}>Patrimônio em Contas Hoje</div>
          <div style={{color:C.gold,fontSize:26,fontWeight:900,fontFamily:"Georgia,serif"}}>{fmtR(totalContas)}</div>
          <div style={{display:"flex",gap:6,justifyContent:"center",marginTop:8,flexWrap:"wrap"}}>
            {contas.filter(c=>c.saldo>0).map(c=><Tag key={c.id} label={`${c.nome}: ${fmtR(c.saldo)}`} cor={c.cor||C.blue}/>)}
          </div>
        </div>
      )}
...
export default function App(){
  ...
  const desp=txMes.filter(t=>t.tipoFluxo!=="Receitas").reduce((s,t)=>s+t.valor,0);
  const saldo=rec-desp;
  const evolucao=meses.slice(-6).map(m=>{const txs=allTx.filter(t=>t.mesAno===m);return{mes:m,receita:txs.filter(t=>t.tipoFluxo==="Receitas").reduce((s,t)=>s+t.valor,0),despesa:txs.filter(t=>t.tipoFluxo!=="Receitas").reduce((s,t)=>s+t.valor,0)};});
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Trebuchet MS',sans-serif",color:C.text,paddingBottom:68,maxWidth:480,margin:"0 auto",position:"relative"}}>
      ...
      <div style={{padding:"13px 13px 6px"}}>
        ...
        {tab==="patrimonio"&&<PatrimonioTab patrimonio={patrimonio} onSavePatrimonio={l=>saveConf("patrimonio",{lista:l},setPatrimonio)} contas={contas}/>}
      </div>
