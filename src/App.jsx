...
// Dado um cartão e uma data de compra, retorna o mesAno da fatura (após fechamento → próximo mês)
const getFaturaMA = (dataCompra, cartao) => {
  const d = new Date(dataCompra + "T12:00:00");
  const dia = d.getDate();
  const fechamento = cartao.diaFechamento || 1;
  // Na ou após o dia de fechamento → vai pra próxima fatura
  if (dia >= fechamento) {
...
// Retorna o mesAno da fatura atual em aberto de um cartão
...
// Dias até o fechamento da fatura
const diasParaFechar = (cartao) => {
  const hoje2 = new Date();
  const fechamento = cartao.diaFechamento || 1;
  const diaAtual = hoje2.getDate();
  if (diaAtual < fechamento) return fechamento - diaAtual;
  // já fechou este mês, próximo fechamento no mês seguinte
  const proximo = new Date(hoje2.getFullYear(), hoje2.getMonth() + 1, fechamento);
  return Math.ceil((proximo - hoje2) / (1000 * 60 * 60 * 24));
};
...
function HomeTab({txMes,receitas,despesas,saldo,orcamento,evolucao,contas,cartoes,objetivos,transactions}){
  ...
  const faturaTotal    = cartoes.reduce((s,c)=>{
    ...
    return s + transactions.filter(t=>t.cartaoId===c.id && getFaturaMA(t.data,c)===fatMA).reduce((a,t)=>a+t.valor,0);
  },0);
...
// ── CARTÕES TAB ───────────────────────────────────────────────────────────────
function CartoesTab({cartoes,onSaveCartoes,allTx,selectedMes,contas,onSaveContas,addTransaction,showToast2}){
  ...
  const[pagarModal,setPagarModal]=useState(null); // cartao
  ...
  const salvar=async()=>{
    if(!form.nome)return;
    setSaving(true);
    const novo={...form,limite:typeof form.limite==="string"?parseVal(form.limite):form.limite};
    const lista=editId?cartoes.map(c=>c.id===editId?{...c,...novo}:c):[...novo,{...novo,id:Date.now().toString()}];
    const listaFinal=editId?cartoes.map(c=>c.id===editId?{...c,...novo}:c):[...cartoes,{...novo,id:Date.now().toString()}];
    await onSaveCartoes(listaFinal);
  ...
  const pagarFatura = async (cartao, faturaMA) => {
    ...
    const total = txFatura.reduce((s,t)=>s+t.valor,0);
    if(total <= 0) return;
    // Registra pagamento como débito da conta corrente
    await addTransaction({
      data: hoje(),
      valor: total,
      fluxo: "Saída",
      tipoFluxo: "Necessidades básicas",
      classificacao: `Fatura ${cartao.nome} ${maToNome(faturaMA)}`,
      observacao: `Pagamento fatura ${maToNome(faturaMA)}`,
      mesAno: getMA(hoje()),
      contaId: pagarConta || null,
      cartaoId: null,
      parcelado: false,
      parcelas: 1,
      valorParcela: total,
      parcelasPagas: [true],
      faturaCartaoId: cartao.id,
      faturaMA,
    });
    setPagarModal(null);
    setPagarConta("");
  ...
  return(
    <div>
      ...
      {cartoes.map(c=>{
        ...
        const alertaFechamento = dias <= 5;

        // Todas as faturas existentes para este cartão
        ...
        // Fatura selecionada para exibir (padrão = atual)
        const faturaVer = expandedCard===c.id+"_prev" ? faturasMAs[faturasMAs.indexOf(faturaAtualMA)-1]||faturaAtualMA : faturaAtualMA;

        const txFaturaVer = allTx.filter(t => t.cartaoId===c.id && getFaturaMA(t.data,c)===faturaVer);
        const totalFaturaVer = txFaturaVer.reduce((s,t)=>s+t.valor,0);
        const jaFoiPaga = allTx.some(t=>t.faturaCartaoId===c.id&&t.faturaMA===faturaVer);
        const limiteUsado = c.limite>0?(totalFaturaVer/c.limite)*100:0;

        // Fatura anterior
        const faturaAnteriorMA = faturasMAs[faturasMAs.indexOf(faturaAtualMA)-1];
        const txFaturaAnterior = faturaAnteriorMA ? allTx.filter(t=>t.cartaoId===c.id&&getFaturaMA(t.data,c)===faturaAnteriorMA) : [];
        const totalAnterior = txFaturaAnterior.reduce((s,t)=>s+t.valor,0);
        const anteriorPaga = faturaAnteriorMA ? allTx.some(t=>t.faturaCartaoId===c.id&&t.faturaMA===faturaAnteriorMA) : true;

        return(
          <div key={c.id} style={{background:`linear-gradient(135deg,${C.card2},${C.card3})`,border:`1px solid ${alertaFechamento?C.amber:c.cor||C.purple}66`,borderRadius:14,padding:"16px",marginBottom:12,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
              ...
              <button onClick={()=>abrirEdicao(c)} style={{background:C.card,border:`1px solid ${C.border}`,color:C.muted,borderRadius:8,padding:"5px 10px",cursor:"pointer",fontSize:11,fontWeight:700,flexShrink:0}}>✏️ Editar</button>
            </div>
            ...
            {faturaAnteriorMA&&!anteriorPaga&&totalAnterior>0&&(
              <div style={{background:C.red+"22",border:`1px solid ${C.red}44`,borderRadius:8,padding:"8px 12px",marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:10,color:C.red,fontWeight:700}}>🔴 Fatura anterior em aberto</div>
                    <div style={{fontSize:12,color:C.text,fontWeight:800}}>{maToNome(faturaAnteriorMA)} · {fmtR(totalAnterior)}</div>
                  </div>
                  <button onClick={()=>{setPagarModal({cartao:c,faturaMA:faturaAnteriorMA,total:totalAnterior});setPagarConta(contas.find(ct=>TIPOS_CORRENTE.includes(ct.tipo))?.id||"");}} style={{background:C.red,border:"none",borderRadius:8,padding:"6px 12px",color:"#fff",fontWeight:700,cursor:"pointer",fontSize:11}}>Pagar</button>
                </div>
              </div>
            )}
            ...
            <div style={{marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <div>
                  <div style={{fontSize:9,color:C.muted,textTransform:"uppercase",letterSpacing:1}}>Fatura Aberta · {maToNome(faturaAtualMA)}</div>
                  <div style={{fontSize:9,color:C.muted}}>Vence dia {c.diaVencimento||"?"} · Fecha dia {c.diaFechamento}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{color:C.red,fontSize:20,fontWeight:900}}>{fmtR(totalFaturaVer)}</div>
                  <div style={{color:C.green,fontSize:10}}>{fmtR((c.limite||0)-totalFaturaVer)} disponível</div>
                </div>
              </div>
              <div style={{background:C.bg,borderRadius:6,height:8,overflow:"hidden",marginBottom:8}}>
              ...
              <div style={{fontSize:9,color:C.muted}}>Limite total: {fmtR(c.limite||0)}</div>
            </div>

            {/* Lançamentos da fatura atual */}
            {txFaturaVer.length>0&&(
              <div style={{marginTop:10,borderTop:`1px solid ${C.border}33`,paddingTop:10}}>
                <div style={{color:C.muted,fontSize:9,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Lançamentos da fatura</div>
                {txFaturaVer.sort((a,b)=>new Date(b.data)-new Date(a.data)).map(t=>{
                  const isParc = t.parcelado && t.parcelas > 1;
                  return(
                    <div key={t.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${C.border}22`,fontSize:11}}>
                      <div>
                        <span style={{color:C.text}}>{t.classificacao}</span>
                        {isParc&&<span style={{color:C.teal,fontSize:9,marginLeft:4}}>1/{t.parcelas}</span>}
            ...
            {faturasMAs.filter(ma=>ma!==faturaAtualMA).length>0&&(
              <div style={{marginTop:10,borderTop:`1px solid ${C.border}33`,paddingTop:8}}>
                <div style={{color:C.muted,fontSize:9,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Faturas anteriores</div>
                {faturasMAs.filter(ma=>ma!==faturaAtualMA).reverse().map(ma=>{
                  const txs = allTx.filter(t=>t.cartaoId===c.id&&getFaturaMA(t.data,c)===ma);
                  const total = txs.reduce((s,t)=>s+t.valor,0);
                  const paga = allTx.some(t=>t.faturaCartaoId===c.id&&t.faturaMA===ma);
                  return total>0?(
                    <div key={ma} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:`1px solid ${C.border}22`}}>
                      <div>
                        <div style={{fontSize:11,fontWeight:700}}>{maToNome(ma)}</div>
                        <div style={{fontSize:9,color:paga?C.green:C.red}}>{paga?"✅ Paga":"⏳ Pendente"}</div>
                      </div>
                      <div style={{display:"flex",gap:8,alignItems:"center"}}>
                        <span style={{color:paga?C.muted:C.red,fontWeight:700,fontSize:12,textDecoration:paga?"line-through":"none"}}>{fmtR(total)}</span>
                        {!paga&&<button onClick={()=>{setPagarModal({cartao:c,faturaMA:ma,total});setPagarConta(contas.find(ct=>TIPOS_CORRENTE.includes(ct.tipo))?.id||"");}} style={{background:C.green,border:"none",borderRadius:6,padding:"3px 8px",color:"#fff",fontWeight:700,cursor:"pointer",fontSize:10}}>Pagar</button>}
                      </div>
                    </div>
                  ):null;
                })}
      ...
      {pagarModal&&(
        <div style={{position:"fixed",inset:0,background:"#000c",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{background:C.card,width:"100%",maxWidth:480,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px"}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>pagarFatura(pagarModal.cartao,pagarModal.faturaMA)} style={{flex:1,background:`linear-gradient(135deg,${C.green},${C.green2})`,border:"none",borderRadius:12,padding:"14px",color:"#fff",fontWeight:800,fontSize:14,cursor:"pointer"}}>✅ Confirmar Pagamento</button>
        ...
        <div style={{position:"fixed",inset:0,background:"#000c",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{background:C.card,width:"100%",maxWidth:480,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px",maxHeight:"90vh",overflowY:"auto"}}>
            ...
            <div style={{background:C.card2,borderRadius:8,padding:"10px 12px",marginBottom:14,fontSize:11,color:C.muted}}>
              💡 Compras no dia {form.diaFechamento||"?"} ou depois vão pra fatura do mês seguinte.
            </div>
            <div style={{marginBottom:16}}>
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Trebuchet MS',sans-serif",color:C.text,paddingBottom:68,maxWidth:480,margin:"0 auto",position:"relative"}}>
      ...
      <div style={{padding:"13px 13px 6px"}}>
        ...
        {tab==="cartoes"&&<CartoesTab cartoes={cartoes} onSaveCartoes={l=>saveConf("cartoes",{lista:l},setCartoes)} allTx={allTx} selectedMes={selectedMes} contas={contas} onSaveContas={l=>saveConf("contas",{lista:l},setContas)} addTransaction={addTx} showToast2={toast2}/>}
